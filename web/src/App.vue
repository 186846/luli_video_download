...
function pickDefaultSub(list) {
  ...
  const pick =
    list.find((s) => String(s.lang).toLowerCase().startsWith('zh')) || list[0]
  selectedSubKey.value = subKey(pick)
}
...
<template>
  <header class="site-header">
    <div class="header-inner">
      <a class="brand" href="/" aria-label="速下 SpeedyDL">
  ...
  <main>
    ...
    <section v-if="video" ref="resultEl" class="result">
    ...
    <section class="section extras" id="extras">
      <h2 class="section-title">更多能力</h2>
      <p class="section-sub">字幕与直链已可用；AI 总结仍为预留</p>
      <div class="card-grid card-grid--2">
        <article class="card platform-card">
          ...
          <h3>字幕下载</h3>
          <p class="card-tags">已支持 · 解析后可选语言</p>
        </article>
        <article class="card card--locked platform-card">
