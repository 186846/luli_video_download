...
flowchart LR
  ...
  Vue -->|"proxy /api"| API[FastAPI_8000]
...
| `downloader.py` | `parse_video` / `download_video` / `resolve_direct_url` / `download_subtitle` |
| `summarizer.py` | 字幕优选 → 文本提取 → LLM / Mock → 摘要/要点/章节 |
| `tasks.py` | 后台线程下载、进度 hook、TTL 清理、并发上限 |
| `thumb_proxy.py` | 校验公网 URL、带 Referer 拉取封面 |
| `config.py` | `FREE_MAX_HEIGHT=720`、`MAX_CONCURRENT_DOWNLOADS`、`FILE_TTL_SECONDS` |
...
| 文件 | 说明 |
|------|------|
| `web/src/App.vue` | 单页聚合：解析、结果、进度、VIP、历史等 |
| `web/src/api/client.js` | API 调用 |
| `web/src/styles/app.css` | 视觉体系 |
| `web/vite.config.js` | `/api` → `http://127.0.0.1:8000` |
...
| 旧 uvicorn 进程占端口 | 重启前清理 8000 端口占用 |
...
3. 新 API 放 `app/main.py` + `downloader.py`；前端只经 `web/src/api/client.js`  
